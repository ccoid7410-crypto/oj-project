...
export class JudgePayloadService {
  ...
  async build(submissionId: string): Promise<JudgePayload> {
    ...
    return {
      ...
      timeLimitMs: clamp(submission.problem.timeLimitMs, MIN_TIME_LIMIT_MS, MAX_TIME_LIMIT_MS),
      memoryLimitMb: clamp(submission.problem.memoryLimitMb, MIN_MEMORY_LIMIT_MB, MAX_MEMORY_LIMIT_MB),
      runnerConfig: {
        fileName: runnerConfig.fileName,
        compileImage: runnerConfig.compileImage,
        compileCmd: runnerConfig.compileCmd,
