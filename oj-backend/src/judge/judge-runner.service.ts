...
import type { JudgeLease, JudgeResult, JudgeTestResult } from './judge-protocol';
...
export class JudgeRunnerService {
  ...
  constructor(
    ...
    const raw = this.config.get<string>('JUDGE_ALLOWED_IMAGE_PREFIXES', '').trim();
  ...
  async run(lease: JudgeLease): Promise<JudgeResult> {
    ...
    try {
      ...
      if (runnerConfig.compileCmd) this.assertImageAllowed(runnerConfig.compileImage!);

      await fs.writeFile(path.join(boxDir, runnerConfig.fileName), lease.sourceCode, 'utf-8');
      ...
      await fs.rm(boxDir, { recursive: true, force: true }).catch(() => undefined);
    }
  }
}
