...
export function normalizeBannerLink(value: string | null): string | null {
  ...
  if (
    ...
    throw new BadRequestException(
      '배너 링크는 인증정보가 없는 http/https URL이어야 합니다.',
    );
  }
  return url.toString();
}
...
export class BannerService {
  ...
  async getPublic(): Promise<PublicBanner> {
    ...
    return {
      enabled: true,
      imageUrl: this.toImageUrl(row.imagePath),
      linkUrl: row.linkUrl,
    };
  }
  ...
  async getAdmin(): Promise<AdminBanner> {
    const row = await this.prisma.siteBanner.findUnique({ where: { id: 1 } });
    if (!row)
      return { enabled: false, imageUrl: null, linkUrl: null, updatedAt: null };
    return {
      enabled: row.enabled,
  ...
  async save(
    ...
    const current = await this.prisma.siteBanner.findUnique({
      where: { id: 1 },
    });
    ...
    if (
      newRelativePath &&
      current?.imagePath &&
      current.imagePath !== newRelativePath
    ) {
