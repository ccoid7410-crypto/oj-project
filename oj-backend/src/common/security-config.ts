...
export function requireJwtSecret(config: Pick<ConfigService, 'get'>): string {
  ...
  if (
    ...
    throw new Error(
      'JWT_SECRET은 예시값이 아닌 32바이트 이상의 무작위 값이어야 합니다.',
    );
  }
  return secret;
}
...
export function requireSecureDatabaseUrl(
  ...
  if (
    ...
    ['oj_password', 'changeme-run-setup-sh'].includes(password)
  ) {
    throw new Error(
      '운영 데이터베이스 비밀번호는 예시값이 아닌 16바이트 이상의 값이어야 합니다.',
    );
  }
}
...
export function requireFrontendOrigin(
  ...
  if (!raw) {
    if (production)
      throw new Error('운영 환경에서는 FRONTEND_URL을 반드시 설정해야 합니다.');
    return 'http://localhost:5173';
  }
  if (raw.includes(','))
    throw new Error('FRONTEND_URL에는 하나의 origin만 설정해야 합니다.');
...
export function resolveCorsOrigins(
  ...
  if (!raw?.trim()) {
    if (production) {
      throw new Error(
        '운영 환경에서는 CORS_ORIGIN 환경변수를 반드시 설정해야 합니다 (콤마로 구분).',
      );
    }
    return ['http://localhost:5173'];
  }
  ...
  if (origins.length === 0)
    throw new Error('CORS_ORIGIN에 유효한 origin이 없습니다.');
  ...
  for (const origin of origins) {
    ...
    if (
      ...
      throw new Error(
        `CORS origin은 http(s) 스킴과 호스트(선택적 포트)만 포함해야 합니다: ${origin}`,
      );
    }
  }
